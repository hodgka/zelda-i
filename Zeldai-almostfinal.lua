...
BoxRadius = 6
InputSize = (BoxRadius*2+1)*(BoxRadius*2+1)*2
...
while true do
   --Statements
	if frames%60==0 then
		--log the fitness by checking the state and number of rupees
...
function initialize()
	...
	local genome = species.genome[pool.currentGenome]
	generateNetwork(genome)
	calculateFitness(checkState(cladder),prevcount)
end


function currentnetworkfitness()
	local species = pool.species[pool.currentSpecies]
	local genome = species.genomes[pool.currentGenome]

	inputs = createinputs)_
	controller = calculateFitness(checkState(cladder),prevcount)
